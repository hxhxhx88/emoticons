...
def search_by_keywords(params)
    ...
    # 
    # 7) Do NOT write something like 
    #  JOIN tags AS t ON t.id = et.tag_id AND (t.name % 'C罗' OR t.name % '中文')
    # since it will NOT use index!
    #
    # 8) A typical whole query will look like the following, from which hopefully you can see what I am doing:
    # WITH hits AS (
    #     SELECT
    #         e.id AS emoticon_id,
    ...
    qs = params[:q].split(',').collect(&:strip).uniq

    # Avoid trivial case
    return [] if qs.length == 0 or (qs.length == 1 and qs[0].length == 0)
...
def search_by_similarity(params)
    # 1) This API requires to install a PostgreSQL extension called pg_similarity, which can be downloaded from
    ...
    # for this.        
    #
    # 2) Should add a column `fingerprint` to the table `images`, which has the type `bigint`.
